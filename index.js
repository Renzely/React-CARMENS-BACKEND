...
//       if (!entry.month || !entry.sku || !entry.expiration || !entry.quantity) {
//         return res.status(400).json({
//           message:
//             "Each expiry entry must have Month, SKU, Expiration, and Quantity.",
//         });
//       }
//     }
...
//   try {
//     // Properly parse with timezone
//     const startDate = dayjs.tz(start, PH_TZ).startOf("day").toDate();
//     const endDate = dayjs.tz(end, PH_TZ).endOf("day").toDate();
...
//   try {
//     // Convert using Asia/Manila timezone
//     const startDateString = dayjs(startDate)
//       .tz("Asia/Manila")
//       .format("YYYY-MM-DD");
//     const endDateString = dayjs(endDate).tz("Asia/Manila").format("YYYY-MM-DD");

//     console.log("Converted to PH date strings:", {
//       startDateString,
//       endDateString,
//     });
...
app.post("/export-inventory-data", async (req, res) => {
  ...
  try {
    ...
    data.forEach((record) => {
      ["ICECREAM", "DAIRY", "MVP"].forEach((versionKey) => {
